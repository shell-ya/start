...
public class DrawAwardDeliveryConsumer implements RocketMQListener<DrawConsumeVO> {
        ...
        if (drawConsumeVO.getDrawAwardVO().getAwardType() == 1) {
            synchronized (this) {
                try{
                    ...
                    log.error("分配编号出现异常",e);
                    drawExec.updateUserAwardState(uId, drawConsumeVO.getDrawOrderVO().getOrderId(), awardId, StarConstants.GrantState.FAIL.getCode());
                }
            }
        }
    ...
    public HandoverReq buildHandOverReq(DrawConsumeVO drawConsumeVO) {
