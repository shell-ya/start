...
public enum RedisKey {
     ...
     * 用户是否已经注册
     */
    REDIS_USER_IS_REGISTERED(203, StarConstants.SERVICE_NAME.concat(".user:info:isRegister:prefix-%s"), -1L, TimeUnit.HOURS),
     ...
     * 秒杀商品库存队列
     */
    SECKILL_GOODS_STOCK_QUEUE(502, StarConstants.SERVICE_NAME.concat(".seckill:stock.queue:%s:time:%s"), -1L, TimeUnit.HOURS),

    /**
     * 秒杀商品随机池
     */
    SECKILL_GOODS_STOCK_POOL(510, StarConstants.SERVICE_NAME.concat(".seckill:stock.cheat:%s:time:%s"), -1L, TimeUnit.HOURS),
    /**
     * 秒杀商品库存数量
     */
     ...
     * 市场交易锁
     */
    MARKET_ORDER_TRANSACTION(508, StarConstants.SERVICE_NAME.concat(".market:transaction:%s"), 5L, TimeUnit.MINUTES),
    ...
    DRAW_AWARD_NUMBER_LOCK(802,StarConstants.SERVICE_NAME.concat(":draw.award.number.lock:%s"),3L,TimeUnit.SECONDS),
    /**
     * 排行榜
     */
//定链价格操作
    ...
    CLOUD_ACCOUNT_STATUS_LIST(1114, StarConstants.SERVICE_NAME.concat(".cloud.account.status.list"), 5L, TimeUnit.SECONDS),
