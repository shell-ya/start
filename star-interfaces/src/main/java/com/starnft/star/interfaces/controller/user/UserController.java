...
import com.starnft.star.interfaces.interceptor.UserId;
...
public class UserController {
    ...
    public RopResponse<Boolean> logout(UserId userId) {
        return RopResponse.success(userCore.logOut(userId.getUserId()));
    }
    ...
    public RopResponse getUserInfo(UserId userId) {
        UserGatheringInfoRes userGatheringInfoRes = userTotalInfoCompose.ObtainUserGatheringInfo(new UserGatheringInfoReq(userId.getUserId()));
        return RopResponse.success(userGatheringInfoRes);
    }

    @ApiOperation("发起实名认证")
    @PostMapping("/userinfo/authentication")
    public RopResponse<Boolean> sponsorAuthentication(UserId userId,
                                                      @Validated @RequestBody AuthenticationNameDTO req) {
        return RopResponse.success(userCore.realNameAuthentication(userId.getUserId(), req));
    }

    @ApiOperation("查询实名认证结果")
    @PostMapping("/userinfo/getauthentication")
    public RopResponse queryAuthentication(UserId userId) {
        return RopResponse.success(userCore.queryAuthentication(userId.getUserId()));
    }
    ...
    public RopResponse<PopupAgreementRes> checkAgreementPopup(UserId userId,
                                                              @RequestParam("scene") Integer scene) {
        return RopResponse.success(userCore.checkAgreementPopup(userId.getUserId(), scene));
    }

    @ApiOperation("保存用户同意协议的版本信息")
    @GetMapping("/userinfo/agreement/insertagreement")
    public RopResponse saveUserAgreementHistoryByUserId(UserId userId,
                                                        @RequestBody AgreementIdRequest request) {
        userCore.saveUserAgreementHistoryByUserId(request.getAgreementId(), userId.getUserId());
